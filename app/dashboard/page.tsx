...
기존 `app/dashboard/components/ControlPanel.tsx` 가 남아있으면 충돌 가능성 있으니:
...
* GPS 기반 현재 위치 + 기상청 지역 연동 구조 반영
* MQTT 실시간 센서 데이터 기반 구조로 변경
* 임시 랜덤값 제거 필요
* EC 실제값 기반 주파수 그래프 연동
* 광량 / 온도 / 습도 실시간 막대 그래프 분리
* 양액 시스템 실제 센서값 기반 동작 구조
* 3색 속도계 → 가시광선 계열 색상 변경
* 속도계 눈금 표시 추가 (1 / 5 / 10)
...
```tsx
<div className="gauge-label label-1">1.0</div>
<div className="gauge-label label-5">5.0</div>
<div className="gauge-label label-10">10</div>
```
...
기존 freqData 렌더 부분 유지하되 실제 EC 기반 데이터 사용:
